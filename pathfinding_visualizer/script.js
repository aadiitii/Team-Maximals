...
$( "#type .dropdown-item").click(function(){
	...
	type = $(this).text();
	Updateheuristics();
	console.log("Heuristic has been changd to: " + type);
});
...
function diagonaldropdown(){
	if(algorithm =="A*" ||algorithm == "Bidirectional A*" ||algorithm == "Greedy Best-First Search" ||algorithm == "Bi-Greedy Best-First Search"){
...
function jumpPointSearch() {
	...
	while (!myHeap.isEmpty()){
		...
		for (var k = 0; k < neighbors.length; k++){
			...
			var newCost = distances[i][j] + Heuristics( type,m,n,i,j );
			if (newCost < costs[m][n]){
				costs[m][n] = newCost;
				myHeap.push([newCost, [m, n]]);
			}
